	...
	do {
		...
		if (rc != size + 1) {
			fprintf(stderr, "%s: i2c wr failed=%d reg=%02x len=%d\n",
				   __FUNCTION__, rc, reg, size); fflush(stderr);
			if (rc < 0)
				return rc;
			return -1;
		}
...
int r82xx_write_reg_mask(struct r82xx_priv *priv, uint8_t reg, uint8_t val,
				uint8_t bit_mask)
{
...
static int r82xx_read(struct r82xx_priv *priv, uint8_t reg, uint8_t *val, int len)
{
	...
	if (rc != len) {
		fprintf(stderr, "%s: i2c rd failed=%d reg=%02x len=%d\n",
			   __FUNCTION__, rc, reg, len); fflush(stderr);
		if (rc < 0)
			return rc;
		return -1;
	}
...
{
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_mux(): freq=%u\n", freq); fclose(f); }
...
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_pll): pll=%u\n", freq); fclose(f); }
...
{
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_sysfreq_sel(): freq=%u, type=%i, delsys=%i\n",freq, type, delsys); fclose(f); }
...
{
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_tv_standard(): bw=%u, type=%i, delsys=%u\n",bw,type,delsys); fclose(f); }
...
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_gain_old(): set_manual=%i, gain=%i\n", set_manual_gain, gain); fclose(f); }
...
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_gain(): manual=%i gain=%i ext=%i lna=%i mix=%i vga=%i \n", set_manual_gain, gain, extended_mode, lna_gain, mixer_gain, vga_gain); fclose(f); }
...
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_bandwidth(): bw=%i rate=%u applied_bw=%u apply=%i\n", bw, rate, *applied_bw, apply); fclose(f); }
...
//		reg_0a = 0x00;
		reg_0a = 0x9f;
		reg_0b = 0x80;
//		fprintf(stderr, "+++++++++++++++++++++++++++++++++++++ zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz\n");
//		fflush(stderr);
		if (apply)
...
fprintf(stderr, " +++++++++++++++++++++++++++ %d (%04x) %d (%04x) %d apply: %d\n", bw, bw,*applied_bw, *applied_bw, priv->int_freq, apply);
fflush(stderr);
//*applied_bw = 300000;
reg_0a = 0xef;
reg_0b = 0xe7;
priv->int_freq = 2300000;
	...
	rc = r82xx_write_reg_mask(priv, 0x0a, reg_0a, 0xff);
	if (rc < 0)
		return rc;
...
FILE* f = fopen("/home/_tweak.txt","at");
if(f) { fprintf(f,"r82xx_set_freq(): freq=%u \n", freq); fclose(f); }

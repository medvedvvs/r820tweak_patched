...
char *get_bits(uint8_t byte, char *bits) {
uint8_t p = 128, pp;
int i;
char c = '1';
memset(bits, '0', 8);
...
typedef struct rtlsdr_tuner_iface {
	...
	int (*set_gain_mode)(void *, int manual);
        int (*set_reg)(void *, uint8_t reg, uint8_t val, uint8_t mask);
        int (*get_reg)(void *, int reg);
...
int r820t_set_gain(void *dev, int gain) {
	rtlsdr_dev_t* devt = (rtlsdr_dev_t*)dev;
FILE*f = fopen("_tweak.txt","at");
if(f) { fprintf(f,"r820t_set_gain\n, gain=%i",gain); fclose(f);}
...
int r820t_get_reg(void *dev, int reg) {
        rtlsdr_dev_t* devt = (rtlsdr_dev_t*)dev;
        return r82xx_read_cache_reg(&devt->r82xx_p,reg);
}
int r820t_set_reg(void *dev, uint8_t reg, uint8_t val, uint8_t mask) {
        rtlsdr_dev_t* devt = (rtlsdr_dev_t*)dev;
        return r82xx_write_reg_mask(&devt->r82xx_p, reg, val, mask);
}
...
static rtlsdr_tuner_iface_t tuners[] = {
	...
	},
	{
		...
		r820t_set_gain_mode, r820t_set_reg, r820t_get_reg
	},
	{
		r820t_init, r820t_exit,
		r820t_set_freq, r820t_set_bw, r820t_set_gain, NULL,
...
int rtlsdr_set_center_freq(rtlsdr_dev_t *dev, uint32_t freq)
{
	...
	if (!r)
        ...
        {
            rtlsdr_set_i2c_repeater(dev, 1);
            dev->tuner->set_reg(dev, 27, dev->saved_27, 255);
            rtlsdr_set_i2c_repeater(dev, 0);
        }
...
int parse(char *message, rtlsdr_dev_t *dev, int fd)
{
    ...
    if ((token == NULL)||(strlen(token)>1) || ( (token[0]!='g') && (token[0]!='s') && (token[0]!='f')) )
    {
        sprintf(response,"?");
...
//            perror("send");
    ...
    char *endptr;
    uint8_t vall = strtol(token, &endptr, 0);
//    if ((token == NULL)||(atoi(token)<5)||(atoi(token)>32))
    if ((token == NULL)||(vall>32))
    {
        sprintf(response,"?");
        if (send(fd, response, 2, 0) < 0) 
        {
...
//        fprintf(stderr,"Get1 reg %x bits: %s\n", 27, get_bits(dev->saved_27, bits));
        rtlsdr_set_i2c_repeater(dev, 1);
        fprintf(stderr,"Set1 reg 0x%03x bits: 0x%x %s mask: %s (0x%03x)\n", reg, val, get_bits(val, bits), get_bits(mask, bits1), mask);
        val = dev->tuner->set_reg(dev, reg, val, mask);
        rtlsdr_set_i2c_repeater(dev, 0);
        sprintf(response,"! %d",val);
        if (send(fd, response, strlen(response), MSG_NOSIGNAL) < 0)
        {
            printf("error sending\n");
            return -1;
        }
